...
const Table = () => {
  const { state: paginationState, contents: paginationContent } =
    useRecoilValueLoadable(bookingPaginationSelector);
  ...
  useEffect(() => {
    if (data.state === "hasError" || paginationState === "hasError") {
      ...
      show({
        message:
          data.state === "hasError" ? `${data.contents}` : paginationContent,
        type: {
          critical: true,
        },
      });
                ...
                return (
                  <TableRow key={index + "booking-table-key"}>
                    ...
                    {!expiredContent?.expired ? (
                      <TableCell dense>
                        <Link className={styles["Table-Link"]} to={newTo}>
                          {i18n.t("View")}
                        </Link>
                        <Link
                          hidden={index !== 0}
